...
% Updated by D. R. Iskander

function [basis, theta, phi] = construct_HSH_basis (degree, sphere_points, dl, real_or_complex)
...
[phi, theta] = cart2hsph(sphere_points(:,1),sphere_points(:,2),sphere_points(:,3));
...
for l = 0:dl:degree   %%% even order only due to the antipodal symmetry!
    % calculate the hemi-spherical harmonics
    Pm = legendre(l,2*cos(theta')-1); % legendre part
    Pm = Pm';
    lconstant = sqrt((2*l + 1)/(2*pi)); % note different constant for hemispherical harmonics
    if (dl==2) 
        center = (l+1)*(l+2)/2 - l;
    else
        if (dl==1) 
