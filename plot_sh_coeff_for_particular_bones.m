...
[number_of_fields, n] = size(field_names);
  figure('name','sh_coeff_for_particular_bones') 
    for i=1:number_of_fields
        ...
        subplot(4,number_of_fields,(i+6)), plot(abs(sbhat1),'r'); ylabel('foot 1 and fsoot2 bhat'); hold on; plot(abs(sbhat2),'b'); hold off
        subplot(4,number_of_fields,(i+9)), plot(xcorr(abs(sbhat1),abs(sbhat2))); ylabel('xcorr(abs(bhat1),abs(bhat2))');
